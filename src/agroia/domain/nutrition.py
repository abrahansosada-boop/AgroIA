...
def optimizar_dieta_pulp(base_datos: dict, req_proteina: float, req_energia: float) -> dict:
    """Motor matemático de optimización lineal de costos."""
    insumos_disponibles = {
    ...
    if not insumos_disponibles:
        return {"exito": False, "error": "Bodega vacía o sin stock."}
    if len(insumos_disponibles) < 2:
        return {"exito": False, "error": "Necesitas al menos 2 ingredientes con stock."}
    ...
    if len(toxicos) >= 2: prob += pulp.lpSum([x[i] for i in toxicos]) <= 11.0, "Colchon_Paranoia"
    ...
    if pulp.LpStatus[prob.status] == "Optimal":
        ...
        for i in insumos:
            if x[i].varValue > 0.01:
                ...
                resultados.append({"Insumo": i.upper(), "Kilos por 100kg": round(x[i].varValue, 2), "Costo ($)": round(costo_ing, 2)})
                ingredientes_usados.append(i)
                detalles_kilos[i] = float(x[i].varValue)

        return {
            "exito": True, "df": pd.DataFrame(resultados), "costo_kg": costo_cien_kg / 100,
            "detalles_ia": {"ingredientes": ingredientes_usados, "kilos": detalles_kilos},
            "proteina_log": req_proteina, "energia_log": req_energia
        }
    else:
        return {"exito": False, "error": "No hay suficientes insumos proteicos o energéticos para lograr esta mezcla."}