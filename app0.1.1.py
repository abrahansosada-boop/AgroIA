    ...
    if 'perfil' not in st.session_state:
        ...
        if st.button("⚖️ AUDITAR MEZCLA"):
                if total_kilos_mezcla > 0:
                    ...
                    st.session_state['mezcla_lista'] = {
                        "total_kilos": float(total_kilos_mezcla),
                        "costo_total": float(costo_tot),
                        "proteina": float(prot_acum)
                    }
                else:
        ...
        if 'mezcla_lista' in st.session_state:
            ...
            if st.button("💾 Procesar Lote y Registrar Gasto", use_container_width=True):
                ...
                detalle_txt = f"Lote de {m['total_kilos']}kg preparado al {m['proteina']:.1f}% de proteína."
                
                try:
                    respuesta = supabase.table("bitacora").insert({
                        "accion": "Preparación de Alimento",
                        "detalle": detalle_txt,
